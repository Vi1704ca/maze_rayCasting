...
class Settings:                                                         #TODO Создаём класс Настройки для настройки игры
    ...
    #def ray_casting(self, sc, player_pos, player_angle, wall_texture):
    #    self.sc = sc
    ...
    def ray_casting(self, sc, player_pos, player_angle, texture):
        ox, oy = player_pos
        xm, ym = mapping(ox, oy)
        cur_angle = player_angle - set.HALF_FOV
        for ray in range(set.NUM_RAYS):
            ...
            for i in range(0, set.WIDTH, set.TILE):
                depth_v = (x - ox) / cos_a
                yv = oy + depth_v * sin_a
                if mapping(x + dx, yv) in set.world_map:
                    break
                x += dx * set.TILE
            ...
            if sin_a >- 0:
                ...
                y = ym
                dy = -1
            for i in range(0, set.HEIGHT, set.TILE):
                depth_h = (y - oy) / sin_a
                xh = ox + depth_h * cos_a
                if mapping(xh, y + dy) in set.world_map:
                    break
                y += dy * set.TILE 
            ...
            offset = int(offset) % set.TILE
            depth *= math.cos(player_angle - cur_angle)
            depth = max(depth, 0.00001)
            proj_height = min(int(set.PROJ_COEFF / depth), 2 * set.HEIGHT)

            wall_column = texture.subsurface(offset * set.TEXTURE_SCALE, 0, set.TEXTURE_SCALE, set.TEXTURE_HEIGHT)
            wall_column = pygame.transform.scale(wall_column, (set.SCALE, proj_height))
            sc.blit(wall_column, (ray * set.SCALE, set.HALF_HEIGHT - proj_height // 2))
    ...
    def draw_sky(self):
        sky_offset = -5 * math.degrees(player.angle) % set.WIDTH
        self.display.blit(floor.floor_texture, (sky_offset, 0))
        self.display.blit(floor.floor_texture, (sky_offset - set.WIDTH, 0))
        self.display.blit(floor.floor_texture, (sky_offset * set.WIDTH, 0))
        #@ sky_texture_scaled = pygame.transform.scale(floor.floor_texture, (set.WIDTH + 10, set.HALF_HEIGHT))
...
class Textures():
    def __init__(self):
        self.floor = 'ceiling.jpg'
        self.wall = 'wall1.jpg'
        self.grass = 'pol2.jpg'
        self.floor_texture = pygame.image.load(settings.directory + '/image/' + self.floor).convert()
...
class Enemy():
    def __init__(self, size, x=0, y=0, hp = 5):
        self.x = x
        self.y = y
        self.size = size
        self.hp = hp
...
while settings.game_active:
    ...
    for event in pygame.event.get():  
        ...
        if event.type == pygame.JOYBUTTONDOWN:
            if event.button == 7:  # Select button
                player.speed = 10
            if event.button == 6:  # Start button
                settings.Fps_hide = not settings.Fps_hide
            if event.button == 2:  # Left joystick press
                settings.Map_hide = not settings.Map_hide
        ...
        if event.type == pygame.JOYAXISMOTION:
            if event.axis == 0:  # Левый стик по оси X
                if event.value < -0.5:
                    settings.go_left = True
                    settings.go_right = False
            ...
            if event.axis == 1:  # Левый стик по оси Y
                if event.value < -0.5:
                    settings.go_up = True
                    settings.go_down = False
                elif event.value > 0.5:
            ...
            if event.axis == 2:  # Правый стик по оси X
                if event.value < -0.5 :
                    settings.turn_left = True
                    settings.turn_right = False
                elif event.value > 0.5:
    ...
    settings.draw_sky()
    settings.draw_floor()
    settings.ray_casting(settings.display, player.pos, player.angle, wall.wall_texture)
